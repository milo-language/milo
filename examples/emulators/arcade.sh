...
#
#   examples/emulators/arcade.sh <rom>   run a specific ROM
#   examples/emulators/arcade.sh          interactive menu of every ROM under roms/
#
...
run_rom() {
  ...
  local src="examples/emulators/$dir/$entry.milo" bin="/tmp/milo-$dir"
  # Rebuild if the binary is missing or any .milo in the core dir is newer.
  if [ ! -x "$bin" ] || [ -n "$(find "examples/emulators/$dir" -name '*.milo' -newer "$bin" 2>/dev/null)" ]; then
