...
# opcode). Usage: examples/emulators/snes/harteSpc.sh 00 e8 cd ...
#        examples/emulators/snes/harteSpc.sh            (runs the implemented set)
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
[ $# -gt 0 ] && OPS=("$@") || OPS=(00 20 40 60 80 e0 ed a0 c0 e8 cd 8d \
  ...
  f5 f6 e7 f7 d5 d6 c7 d7 f9 fb db d9 \
  1b 5b 3b 7b bb 9b de)
pass=0; fail=0
for op in "${OPS[@]}"; do
  js="$D/$op.json"
  if [ ! -f "$js" ]; then printf '%-6s MISSING\n' "$op"; continue; fi
  bun run examples/emulators/snes/harteConvSpc.ts "$js" /tmp/hs.flat 2>/dev/null || true
  out=$(MILO_RUN_MEM_MB=3000 bun run src/main.ts run examples/emulators/snes/runHarteSpc.milo /tmp/hs.flat 2>/dev/null | grep -E 'passed|first fail' || true)
