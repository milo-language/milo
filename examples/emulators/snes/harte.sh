...
# Usage: examples/emulators/snes/harte.sh ea a9 aa ...
#        examples/emulators/snes/harte.sh            (runs the implemented set)
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
for op in "${OPS[@]}"; do
  for mode in e n; do
    ...
    bun run examples/emulators/snes/harteConv.ts "$js" /tmp/h.flat 2>/dev/null || true
    out=$(MILO_RUN_MEM_MB=3000 bun run src/main.ts run examples/emulators/snes/runHarte.milo /tmp/h.flat 2>/dev/null | grep -E 'passed|first fail' || true)
