...
# Usage: examples/apps/genesis/harte.sh NOP MOVE.w ADD.l ...
#        examples/apps/genesis/harte.sh            (runs a default supported set)
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
for op in "${OPS[@]}"; do
  ...
  bun run examples/apps/genesis/harteConv.ts "$gz" /tmp/h.flat 2>/dev/null || true
  out=$(MILO_RUN_MEM_MB=3000 bun run src/main.ts run examples/apps/genesis/runHarte.milo /tmp/h.flat 2>/dev/null | grep -E 'passed|fail' || true)
