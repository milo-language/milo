...
for op in "${OPS[@]}"; do
  ...
  bun run examples/apps/genesis/harteConvZ80.ts "$D/$op.json" /tmp/z.flat 2>/dev/null
  out=$(MILO_RUN_MEM_MB=2000 bun run src/main.ts run examples/apps/genesis/runHarteZ80.milo /tmp/z.flat 2>/dev/null | grep -E 'passed|fail' || true)
