...
| `bin/menu` | Fullscreen SDL menu (see `examples/apps/menu.milo`); prints `<system>\t<rompath>`. |
...
The menu, font, and SDL glue live in `examples/apps/menu.milo` and
`examples/apps/shared/{menucore,font,sdl}.milo`. Emulators stay independent
...
```sh
examples/apps/retro/build.sh          # builds bin/ (emulators at --release)
examples/apps/retro/launch.sh         # windowed
```
...
2. **Get the code + build:**
   ...
   examples/apps/retro/build.sh                 # bin/ built natively on the Pi
   ```
3. **Lay out the console dir** (`launch.sh` defaults `RETRO_HOME` to the repo
   ...
   ```sh
   mkdir -p ~/retro && cp -r examples/apps/retro/bin ~/retro/
   cp examples/apps/retro/launch.sh ~/retro/
...
4. **Pair a controller:**
   ```sh
   examples/apps/retro/pair-controller.sh
   ```
5. **Boot into it:** `sudo raspi-config` → System Options → Boot/Auto Login →
   **Console Autologin**. Then install the service:
   ```sh
   sudo cp examples/apps/retro/retro-console.service /etc/systemd/system/
...
- **Menu look** — colors, layout, and the 5×7 font live in
  `examples/apps/shared/{menucore,font}.milo`.
