...
// docs/std/ is generated by scripts/gen-std-docs.ts from std's doc-comments, so it is
// self-healing by construction — `update-when` and `last-verified` describe nothing a
// human maintains, and the header would be 89 more lines for the generator to emit and
// tests/stdDocs.test.ts to keep in sync. Warning on all 89 every regen is the kind of
// noise that gets a lint switched off wholesale, taking the fixable cases with it.
const isGeneratedDoc = (f: string) => f.startsWith("docs/std/");
const isDoc = (f: string) =>
  f.startsWith("docs/") && f.endsWith(".md") && !isGeneratedDoc(f);
