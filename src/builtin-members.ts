...
export const BUILTIN_MEMBERS: Record<BuiltinReceiver, BuiltinMember[]> = {
  ...
  string: [
    ...
    { name: "charAt", sig: "(i: i64): string", note: "BYTE index, not a character — charAt(1) of \"aéb\" is half of the é and is not valid UTF-8. Iterate with `for c in s.codePoints()` for characters" },
    ...
    { name: "padStart", sig: "(targetLen: i64, pad: string): string", note: "targetLen counts BYTES, so a multibyte string pads short of the visual width you asked for" },
    { name: "padEnd", sig: "(targetLen: i64, pad: string): string", note: "targetLen counts BYTES, like padStart" },
