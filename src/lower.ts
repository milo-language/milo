...
class LowerCtx {
  ...
  lowerProgram(program: Program): HIRModule {
    ...
    for (const s of program.structs) {
      ...
      structs.push({ name: s.name, fields: info.fields.map(f => ({ name: f.name, type: f.type, ...(f.cOpaque ? { cOpaque: true } : {}), ...(f.cName ? { cName: f.cName } : {}) })), isExtern: info.isExtern, cLayout: info.cLayout });
    }
  ...
  private lowerExprRaw(expr: Expr, type: TypeKind): HIRExpr {
    switch (expr.kind) {
      ...
      case "Call": {
        ...
        const cfnType = this.c.cfnCalls.get(expr);
        if (cfnType) {
          const cargs: HIRArg[] = expr.args.map(arg => {
          ...
          return {
            kind: "CFnCall",
            callee: { kind: "Ident", name: expr.func, type: cfnType, span: expr.span },
            args: cargs,
            type,
            span: expr.span,
          };
