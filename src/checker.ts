...
export class TypeChecker {
  ...
  private checkProgram(program: Program): void {
    ...
    const host = this.programPassHost();
    ...
    checkThreadBoundary(host, program, view);
  ...
  // The checker as the whole-program passes see it (`checker-program-passes.ts`). An
  // explicit literal rather than `this` so the members stay private and the seam is
  // readable here: what a pass can reach is this list and nothing else.
  private programPassHost(): ProgramPassHost {
    return {
      exprTypes: this.exprTypes,
      functions: this.functions,
      monomorphizedFns: this.monomorphizedFns,
      ...
      cfnFieldCalls: this.cfnFieldCalls,
      interfaceMethodCalls: this.interfaceMethodCalls,
      error: (msg, span, hint) => this.error(msg, span, hint),
      warn: (code, msg, span, hint, len) => this.warn(code, msg, span, hint, len),
      show: (t) => this.show(t),
