...
export class TypeChecker {
  ...
  // impl that releases something, or `@noCopy` on a handle, anywhere inside the type.
  ...
  private resourceKind(ty: TypeKind, seen: Set<string> = new Set()): { kind: "Drop" | "@noCopy"; via: TypeKind } | null {
    switch (ty.tag) {
      case "struct": {
  ...
  private copyOutUnavailable = new Map<string, { kind: "Drop" | "@noCopy"; via: TypeKind; arg: TypeKind }>();
  ...
  private copyOutReason(what: string, b: { kind: "Drop" | "@noCopy"; via: TypeKind; arg: TypeKind }): string {
