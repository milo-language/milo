...
test("every flag main.ts parses is documented, and no documented flag is unparsed", () => {
  ...
  // A table flag is written as it is typed (`--target=<name>`); match on the name. A flag
  // one subcommand reads is documented under that command instead of in the options block.
  const written = [...OPTIONS.map(o => o.flag), ...[...COMPILER_COMMANDS, ...PACKAGE_COMMANDS].flatMap(c => (c.flags ?? []).map(f => f.flag))];
  const documented = new Set(written.map(f => /^(--[a-z-]+)/.exec(f)?.[1]).filter(Boolean) as string[]);
  const undocumented = [...parsed].filter(f => !documented.has(f)).sort();
  expect(undocumented).toEqual([]);
});

test("help, --help and -h print the banner and exit 0", () => {
...
test("no banner row runs its description into the left column", () => {
  ...
  const rows = [
    ...[...COMPILER_COMMANDS, ...PACKAGE_COMMANDS].flatMap(c =>
      c.hidden ? [] : [{ left: c.usage, described: c.summary.length > 0 }, ...(c.extraRows ?? []).map(e => ({ left: e.usage, described: true }))]),
    ...OPTIONS.map(o => ({ left: o.flag, described: o.help.length > 0 })),
  ];
...
test("a flag or path that is not a file is a usage error, not a stack trace", () => {
  // parseArgs takes the first unrecognized argument as the source path, so
  // `milo safety --level do178c-a f.milo` used to reach readFileSync("--level") and die
  // with a raw Bun stack trace.
  const r = spawnSync("bun", ["run", join(ROOT, "src", "main.ts"), "safety", "--level", "do178c-a", "examples/hello.milo"], { cwd: ROOT, encoding: "utf-8" });
  expect(r.status).toBe(1);
  expect(r.stderr).toContain("usage: milo safety <file> --safety=<level>");
  expect(r.stderr).not.toContain("ENOENT");
  const missing = spawnSync("bun", ["run", join(ROOT, "src", "main.ts"), "check", "no-such-file.milo"], { cwd: ROOT, encoding: "utf-8" });
  expect(missing.status).toBe(1);
  expect(missing.stderr.trim()).toBe("error: no such file or directory: no-such-file.milo");
  // A flag ahead of the path is a flag, not the path.
  const flagFirst = spawnSync("bun", ["run", join(ROOT, "src", "main.ts"), "check", "--json", "examples/hello.milo"], { cwd: ROOT, encoding: "utf-8" });
  expect(flagFirst.status).toBe(0);
  expect(JSON.parse(flagFirst.stdout).ok).toBe(true);
}, 60000);
