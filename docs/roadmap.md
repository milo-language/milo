...
- **JSON runs in the playground again** (2026-09-20): the emit-js parity baseline dropped 313 → 270. `std/json` keeps each number in a per-document table instead of punning the f64's bits through a raw pointer, and the JS backend learned four things it was silently getting wrong or refusing: an atomic on a global is plain arithmetic on one thread; `&mut self.field` and `&mut v[i]` primitive arguments are accessor boxes (their writes were dropped); `&mut` primitive closure parameters are boxes too; and i64 `& | ^` work on 32-bit halves (JS's operators are int32, so `(docId << 32) | idx` lost its high word). Also found there: the checker's global-initializer ordering never saw a call through a free fn (`Call.func`, not `callee`), which only emit-js could observe
...
- **Test framework**: `@expect:`/`@error:` annotations, `milo test` runner — <!-- stat:fixtures -->731<!-- /stat --> fixtures, <!-- stat:error-fixtures -->422<!-- /stat --> error fixtures, <!-- stat:prove-fixtures -->38<!-- /stat --> prove fixtures
