...
- [~] **Windows port** — *core works and is CI-verified; the async/IOCP tier does not.* `getHostTarget()` used to fall through to the Linux entry for any non-darwin host, so Windows silently claimed `x86_64-unknown-linux-gnu` and emitted ELF-targeting IR — it didn't fail, it lied. `windows-x64`/`windows-arm64` are now real targets (`x86_64-pc-windows-msvc`), and CI's `windows-latest` job runs **the fixture suite itself** — **401/441 (91%)** compile and run as native PEs on real Windows. The other 40 carry `// @skip-os: win32` with the reason inline, so the skip list is the remaining port work, item-by-item, rather than a number in a doc. See [breaking-changes.md](breaking-changes.md) for the `std/os` → `std/platform` relocation this required.
  ...
  - [ ] **Winsock proper** (5 fixtures) — sockets link, but need `WSAStartup` at init and `WSAGetLastError` for errors: Windows sockets do not set `errno`, so `std/net`'s EAGAIN/EINPROGRESS checks cannot work as written. The TLS paths additionally need an OpenSSL built for the target sysroot; `xwin` ships the CRT and SDK, not third-party libraries. There is also a **type-width problem** the extern audit surfaced: `socket`/`accept` return `SOCKET` (a `UINT_PTR`, 8 bytes) and take `SOCKET` params, but `std/os` declares them `i32` — the whole net layer is i32-fd-based. MS documents that `SOCKET` values fit in 32 bits, so it works in practice, but it is a genuine ABI mismatch. This is why `scripts/audit-extern-returns.ts` is NOT yet wired into the Windows CI job (it correctly flags these); turn it on there once this tier lands.
  - [ ] **`std/crypto`** (4), **`std/pty`** → ConPTY (2), **`std/regex`** (1), **`fork`/`waitpid`/`kill`** (5) → `CreateProcess`, which has no fork semantics to emulate. `std/environ` and `std/sysinfo` also have no Windows arm yet (`_environ`/`_putenv_s`, `GetSystemInfo`/`GetComputerNameA`) — no fixture reaches them, but `examples/terminal/sysmon.milo` does.
