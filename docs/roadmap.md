...
- [~] **Windows port** — *core works and is CI-verified; the async/IOCP tier does not.* `getHostTarget()` used to fall through to the Linux entry for any non-darwin host, so Windows silently claimed `x86_64-unknown-linux-gnu` and emitted ELF-targeting IR — it didn't fail, it lied. `windows-x64`/`windows-arm64` are now real targets (`x86_64-pc-windows-msvc`), and CI's `windows-latest` job runs **the fixture suite itself** — **401/441 (91%)** compile and run as native PEs on real Windows. The other 40 carry `// @skip-os: win32` with the reason inline, so the skip list is the remaining port work, item-by-item, rather than a number in a doc. See [breaking-changes.md](breaking-changes.md) for the `std/os` → `std/platform` relocation this required.
  ...
  - [ ] **Winsock proper** (5 fixtures) — sockets link, but need `WSAStartup` at init and `WSAGetLastError` for errors: Windows sockets do not set `errno`, so `std/net`'s EAGAIN/EINPROGRESS checks cannot work as written. The TLS paths additionally need an OpenSSL built for the target sysroot; `xwin` ships the CRT and SDK, not third-party libraries.
  - [ ] **`std/crypto`** (4), **`std/pty`** → ConPTY (2), **`std/regex`** (1), **`std/dl`** → `LoadLibrary` (1), **`fork`/`waitpid`/`kill`** (5) → `CreateProcess`, which has no fork semantics to emulate. `std/environ` and `std/sysinfo` also have no Windows arm yet (`_environ`/`_putenv_s`, `GetSystemInfo`/`GetComputerNameA`) — no fixture reaches them, but `examples/terminal/sysmon.milo` does.
  - [ ] **`std/os` fd calls are wrong on Windows** — found by running the c-decl guards natively for the first time. `read`/`write` link through the UCRT's oldnames shim to `int _read(int, void *, unsigned int)`, but `std/os` declares 64-bit count and return. Win64 leaves the upper half of RAX undefined for an `int` return, so a `-1` can arrive as a large positive `i64`. Their `@cSig`s are now scoped `"darwin,linux"`, which records the gap instead of asserting a header Windows lacks; the fix is a Windows arm over `_read`/`_write`/`_lseeki64`, which needs those externs to move into the platform split (another relocation — see [breaking-changes.md](breaking-changes.md)).
  - [ ] **cross-target C decl verification** — `verifyCDecls` skips itself whenever target ≠ host, so the `@cLayout`/`@cSig` guards that would catch LLP64 mistakes (`long` is 4 bytes on Windows, 8 everywhere else Milo hosts) are silently absent on every cross-compile. It needs a target sysroot, not the host's headers. `scripts/windows-sweep.ts` covers execution but not this.
