...
- **Test framework**: `@expect:`/`@error:` annotations, `milo test` runner — <!-- stat:fixtures -->731<!-- /stat --> fixtures, <!-- stat:error-fixtures -->420<!-- /stat --> error fixtures, <!-- stat:prove-fixtures -->38<!-- /stat --> prove fixtures
...
- [x] **Error boxing** — shipped 2026-09-21: the prelude `Error` interface (`message(self: &Self): string`), and `?` in a fn returning `Result<T, Heap<Error>>` boxes any satisfying struct or enum error, or a `string` as `Message` (`tests/fixtures/errorBoxing.milo`). Open: `.context("…")` chaining and `OsError` (backlog A4)
