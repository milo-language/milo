...
last-verified: 2026-09-21 (hygiene: three stale Planned checkboxes flipped, @pure moved to Completed, tuples row rewritten; every path returns; the import list restricts; earlier: prover: builtin container contracts, len kept across len-preserving loops, honest verdicts on contract-less havocs; earlier: self-host endgame decided proof-only and src-milo frozen; packages ecosystem at 8 published with an install-and-build gate; green main; counts re-measured)
-->
...
- **Only `pub` fns are exported symbols** (2026-09-21) from `build-lib` / `emit-obj`, as the header already declared only them; a non-`pub` entry-file fn is `internal` now, so C cannot hand-declare and link a private helper (`exportedFnNames` beside `userFnNames`, which keeps its sret and attribution jobs)
- **`Process.wait()` no longer wedges the scheduler** (2026-09-21): in a green task it polls `waitpid(WNOHANG)` with a green sleep between polls, so the other tasks keep running while a child runs; the main context keeps its blocking `waitpid`. The SIGCHLD-owned reaper the backlog designed stays a follow-up (C4)
- **An enum that reaches itself by value through a struct is rejected** (2026-09-21): `enum E { X(A) }` with `struct A { e: E }` has infinite size, and it compiled with `E`'s payload sized before `A`'s size was known, so every `E` built spilled 32 bytes past itself (the ASan sweep's one red fixture). The struct rule now walks enum payloads too; the fixture that relied on it was rewritten without the cycle
- **`unchecked-ffi-contract`** (2026-09-21): the #27 decision, made as a lint. A `requires` on an `unsafe`-bodied fn is dropped at `-O2`, so the body must check the same values itself; the off-by-default warning lists every such fn whose `requires` names parameters no `if` or `assert` in the body mentions. Its first sweep found two more real ones beyond the crypto and pool cases already fixed: `Pool.free` wrote through an unchecked block address (now asserted) and `Channel.new` accepted a non-positive capacity (now an `Err`). The rest of the set (`bumpNew`, `Pty.spawn`/`wait`, `Child.writeStdinStr`) are state preconditions whose failure is a benign errno, which is why the lint is opt-in
...
- **`milo test --contracts`** (2026-09-21): property tests written by the contracts. Every fn with `requires`/`ensures` over integers, bools, floats or strings gets a `testContract_<fn>` that draws inputs (edges first; a `requires key.len == 32` is drawn at that length), filters by `requires`, and checks `ensures` with `result` bound, printing the refuting inputs. Its first sweep of std found five contracts that promised more than the fn delivers (`strRepeat`, the pads, `Pool.new` and `inflate.zeros` admit a count that exhausts memory; `strCharAt` needs a character boundary) and they were tightened. `src/contract-tests.ts`, gate `tests/contractTests.test.ts`
...
- **HashMap in-place access** (2026-09-21): `m.modify(k, (v: &mut V) => …): bool` runs the callback on the stored value where it sits, `m.getOrInsertWith(k, init): bool` fills a missing key; the map is borrowed for the callback. The group-by and count idioms no longer clone, mutate and re-insert
- **`&mut` payload views** (2026-09-21): matching through a `&mut Enum` binds every payload as a `&mut T` view, so `Node.Leaf(v) => { v = v + 1 }` updates in place (backlog #41 closed; replace-rebuild-assign is no longer the only way). Found on the way and closed: reassigning a subject inside an arm that still held a `&` payload view was accepted and read freed memory; the subject is now borrowed for the arm
- **`milo fix`** (2026-09-21): `check --json` (schema 2) carries a `fix` on every diagnostic with one right resolution (a missing `&mut`, an unimported or unused name, a bare `embedFile`/`targetOs`, an unused `unsafe`, a string that meant to interpolate), and `milo fix <entry>` applies them in every file the check reaches. One table (`src/fixes.ts`) feeds the JSON, the LSP quickfixes and the command, so a fix that works in the editor works on the command line. Choices (clone or borrow, `pub` in another file) stay hints
- **Object cache** (2026-09-21): `~/.milo/cache/obj/<sha256>.o`, keyed by a codegen unit's IR text plus the compiler's version line and flags, so clang runs only for units whose IR changed. The CGU packing is sticky per program (`placement/`), since a size-driven repack moved every function for a one-line edit. milojs-engine, one function edited: 22.2s → 13.4s (7 of 8 units served); unchanged rebuild 10.5s, which is now all frontend. `MILO_OBJ_CACHE=0` disables; MILO_VERBOSE=1 reports hits
- **`std/json` off raw pointers** (2026-09-20): `std/json` keeps each number in a per-document table instead of punning the f64's bits through a raw pointer to fit the 24-byte node, which keeps the node pointer-free. Found on the way: the checker's global-initializer ordering never saw a call through a free fn (`Call.func`, not `callee`), so a global built by `arenaNew<Node>()` could read the arena counter before its declaration
- **Pointer payloads make an enum move-tracked** (2026-09-20): `enum Slot { Some(*u8), Empty }` follows the struct rule, so an owning handle wrapped in an enum is no longer duplicable; `@copy` on the enum opts out, as it does for a struct. Closed before any enum in std, the examples, the packages or milojs carried one
- **The import list restricts**: an exported name a file never listed in `from "m" import { … }` is not in scope there (`'strlen' is not imported`, with the line to add). Before 2026-09-20 the list was validated and then ignored, so `from "std/os" import { getenv }` made every std/os export callable and `import { }` imported everything. `milo fix` adds what a program is missing; std, the examples, the packages and milojs (389 sites) were brought over with it
...
- **Every path returns**: a non-`void` fn or closure whose body can reach its end without a `return` is rejected (`if` without `else`, a bare tail expression, a `match` that returns in every arm of a subject it does not cover). `main` alone keeps C's implicit `return 0`. Before 2026-09-20 an `i32` fn silently returned 0, a closure returned a zero of its type (a null `string`), and any other type reached clang as a block with no terminator
...
- **Builtin containers have contracts**, stated in Milo as the `ensures` each method would carry if it were library code (`BUILTIN_CONTRACTS_SRC` in `src/verify.ts`, consumed by the same frame machinery as a user `&mut` callee). A method absent from the table still havocs its receiver and, by the rule above, yields `unknown`. Each entry is checked against the runtime by `tests/prove/builtinContainerContracts.milo`, which the prove driver both proves and executes under `--debug`. The table is keyed by method name and applied only when the receiver is known to be a Vec/string/array, so a user type's own `push` is untouched. A loop consults the same table: when every write to a place in the body keeps `len` (`[i] = x`, `swap`, `sort`, a callee with `ensures v.len == old(v.len)`), the loop havoc keeps the place's `len` symbol and frees only its contents, which is what lets `for i in 0..n { v[i] = x }` be followed by a proven `v.len`-dependent precondition; one `push` in the body and the length is as unknown as before:
...
- **Test framework**: `@expect:`/`@error:` annotations, `milo test` runner — <!-- stat:fixtures -->737<!-- /stat --> fixtures, <!-- stat:error-fixtures -->423<!-- /stat --> error fixtures, <!-- stat:prove-fixtures -->38<!-- /stat --> prove fixtures
...
- [x] **CI runs on linux-arm64 (2026-09-21)**, `ubuntu-24.04-arm` in the `test-linux` matrix, and the first run found the hole: `epoll_event` is `__attribute__((packed))` on x86_64 only (12 bytes, `data` at offset 4) and padded to 16 bytes on aarch64, and `std/event.linux.milo` hardcoded the x86_64 shape, so every registration on arm64 watched fd 0 and every green-IO fixture hung. Fixed with `@targetArch()`, a new compile-time builtin beside `@targetOs()`. The `free(): invalid pointer` milojs saw on the same target has not been reproduced yet; the runner is what will
- [ ] ~~**CI has no linux-arm64 runner.**~~ (superseded above) The matrix is macOS-arm64, linux-x64 and Windows-x64. Nothing in this repo exercises linux-arm64 codegen, and the gap is not theoretical: milojs's release job — which builds against this compiler's rolling `latest` — has aborted on that target since 2026-08-15 with `free(): invalid pointer` on a hello-world, while linux-x64 and darwin-arm64 are clean. A `ubuntu-22.04-arm` runner belongs in `ci.yml`, and the failure should be reproduced here rather than diagnosed downstream
...
- [ ] **Struct destructuring** — `let { pos, err } = scan(s)` binds fields by name (a rename spelling `{ pos: p }`), desugared to a hidden temporary plus field reads so the existing move rules apply unchanged. Decided 2026-09-21 instead of tuples: see Retired
- [x] **Combinators on slices and arrays** — shipped 2026-08-17 (`tests/fixtures/{slice,array}Combinators.milo`; `fold` included, `SLICE_COMBINATORS` in `src/types.ts`). `take`/`skip`/`zip` are declined — see Retired. Lazy/fusing adapters are deliberately out
- [x] **Error boxing** — shipped 2026-09-21: the prelude `Error` interface (`message(self: &Self): string`), and `?` in a fn returning `Result<T, Heap<Error>>` boxes any satisfying struct or enum error, or a `string` as `ErrorMessage` (`tests/fixtures/errorBoxing.milo`), and `r.context("…")` wraps the boxed error in an `ErrorContext` whose message reads outermost first (`tests/fixtures/errorContext.milo`). `OsError` is the separate row below
...
- [x] **`@unsafe fn` declarations** (language-reference §`@unsafe fn`)
- [ ] **`--deny-unsafe`** for user code; `unsafe` visibility in the LSP
...
- **JS backend (`milo emit-js`)** - removed 2026-09-21: a second implementation of the language's semantics (duck-typed interface dispatch, no destructors, a name clash with any fn called `process`) that cost a parity fix on most language changes. LLVM's `--target=wasm64` is the browser target; the docs-site playground and the in-browser emulator demos are paused until the emulators build for it
...
- **Tuples** — declined 2026-09-21 on a census: of 140 two/three-field structs across std, the examples, milojs and dapweb, 53 exist only to return values, and most carry a name that reads better than `.0`/`.1` (`RegexMatch`, `FileInfo`, `AesGcmResult`); 16 fns use a `&mut` scalar as a second return. The pull is ~20-30 sites, the cost a new type through every layer (layout, checker, moves, fmt, LSP, JS backend, spec). What those sites want is the binding form, so struct destructuring ships instead
