...
- [~] **Windows port** — *core works and is CI-verified; the async/IOCP tier does not.* `getHostTarget()` used to fall through to the Linux entry for any non-darwin host, so Windows silently claimed `x86_64-unknown-linux-gnu` and emitted ELF-targeting IR — it didn't fail, it lied. `windows-x64`/`windows-arm64` are now real targets (`x86_64-pc-windows-msvc`), CI runs a `windows-latest` job, and **364/402 fixtures (90.5%) compile and run correctly** as native PEs.
  ...
  - [ ] **IOCP async tier** (~11 fixtures) — the expensive one, and not a translation: kqueue/epoll report *readiness* ("ready, go read"), IOCP reports *completion* ("the read you started finished"), so `std/runtime` restructures around completion. Until then `eventPoll`/`eventRegisterRead` abort with a named message, and green IO on a non-socket fd refuses rather than deadlocking (`ioctlsocket` is sockets-only, so without that guard `read` blocked the scheduler thread forever with no output).
  - [ ] **Winsock proper** — sockets link, but need `WSAStartup` at init and `WSAGetLastError` for errors: Windows sockets do not set `errno`, so `std/net`'s EAGAIN/EINPROGRESS checks cannot work as written.
  - [ ] **`std/crypto`** (4), **`std/pty`** → ConPTY (2), **`std/regex`** (1), **`std/dl`** → `LoadLibrary` (1), **`fork`** (2) → `CreateProcess`, which has no fork semantics to emulate.
