...
- [ ] **`#[c_layout]` — extern struct layout verification** — a declared `extern struct` layout is taken on faith; a wrong offset silently reads the neighbouring field and returns plausible garbage. `#[c_layout("struct stat", "sys/stat.h")]` would have the compiler emit a throwaway C TU of `_Static_assert(offsetof(...) == N)` per field and compile it with the system `cc` at build time. Same faith applies to hand-written `extern fn` decls; `unsafe` deliberately doesn't cover either (tracks provenance, not layout/effects). Prerequisite-free stepping stone to a full `@cImport`. See `~/git/node/src/milo/MILO_PAINPOINTS.md` #8.
