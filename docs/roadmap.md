...
- [~] **Windows port** — *core works and is CI-verified; the async/IOCP tier does not.* `getHostTarget()` used to fall through to the Linux entry for any non-darwin host, so Windows silently claimed `x86_64-unknown-linux-gnu` and emitted ELF-targeting IR — it didn't fail, it lied. `windows-x64`/`windows-arm64` are now real targets (`x86_64-pc-windows-msvc`), and CI's `windows-latest` job runs **the fixture suite itself** — **405/440 (92%)** compile and run as native PEs on real Windows. The other 35 carry `// @skip-os: win32` with the reason inline, so the skip list is the remaining port work, item-by-item, rather than a number in a doc. See [breaking-changes.md](breaking-changes.md) for the `std/os` → `std/platform` relocation this required.
