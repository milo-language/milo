...
| 9 | **Fixed-size arrays are neither Copy nor cloneable** | M | M | `[u8; 16]` (an IPv6 address) can't be passed to two functions: the first call MOVES it, and the compiler's own hint — "clone it at the point of transfer: 'x.clone()'" — suggests a method arrays don't have (`type '[u8; 16]' has no method 'clone'`). So the diagnostic names a fix that cannot be applied. Hit while writing `tests/fixtures/tcpIpv6.milo`, which now calls `ip6("::1")` twice to dodge it. An array of Copy elements should arguably BE Copy (it is a fixed-size value with no heap); failing that, it needs `clone`. Decide which, then the hint stops lying. | `types.ts isCopy` (array arm), `checker.ts` move-hint text |
