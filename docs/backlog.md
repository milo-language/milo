...
|---|------|-----|--------|----------------|-----|
| 1 | **Annotate extern decls with `@cSig`/`@cLayout`** | M | L | Mechanism shipped; 21 of 215 std extern fns annotated, plus node-milo's `Stat`/`Timespec`/`Timeval` (which lets ~18 of its 23 hand-written `_Static_assert`s in `entry.c` retire). The rest are unverified claims. Mechanical, and each is a real check — payoff lands when a data model differs (`long` is 4 on LLP64, making a correct-today `i64` wrong). Do it per module, not in one sweep. | `std/*.milo`, `~/git/node/src/milo/` |
