...
| F2 | **Unknown-attribute validation** | M | L | `@derive`/`@send`/`@sync`/`@cLayout` are matched by name and **anything unrecognized is silently ignored** — no validation pass exists. So `@clayout(...)` (wrong case) or `@drive(Clone)` quietly does nothing, which is the same silent-failure class F1 just closed. Attribute names are a closed set; reject unknown ones. | `checker.ts:936-944` |
...
| F6 | **`safety.ts` walker never descends into `is` / if-expressions** | H | L | Found by the tsc gate. The walker's arms named fields that don't exist — `IsExpr.expr` (it's `operand`) and `IfExpr.thenBranch`/`elseBranch` (they're `thenBody`/`elseBody`, and are `Stmt[]`, so they need the stmt visitor, not `ex`). Both silently passed `undefined` into a walker that no-ops on falsy input, so the **float check and call-graph extraction have never looked inside an `is` operand or an if-expression's branches** — e.g. a float op inside `if c { a } else { b }` as an expression goes unreported. Behavior preserved + documented in place rather than fixed blind, since traversing changes what safety analysis sees. Fixing it needs its own change + fixtures for each missed construct. | `src/safety.ts` |
