...
|---|------|-----|--------|----------------|-----|
| F1 | ~~**`@cLayout` — verify `extern struct` field offsets**~~ ✅ shipped 2026-07-16. | H | L→M | The compiler took a declared `extern struct` layout on faith: a wrong offset silently read the neighbouring field and returned plausible garbage. Now `@cLayout("struct timespec", "time.h")` emits a throwaway C TU of `_Static_assert(offsetof(...) == N)` **and** `sizeof(field) == M` per field, compiles it with the system `cc` at build, discards it. Struct size checked `>=` not `==` (prefix decls are legit — std's `Stat` stops at `st_blksize`). Skipped for bare-metal (freestanding, cross-compiled). **Remaining:** opt-in only — nothing tells you an unannotated extern struct is unverified (see F2), and the 341 hand-written `extern fn` decls are still unchecked. Stepping stone to `@cImport`. | `codegen.ts cLayoutGuards`, `main.ts verifyCLayouts`; `~/git/node/src/milo/MILO_PAINPOINTS.md` #8 |
| F2 | ~~**Unknown-attribute validation**~~ ✅ shipped 2026-07-16. | M | L | Attributes were matched by name with **anything unrecognized silently ignored** — `@drive(Eq)` compiled clean and did nothing. Also found: enums parse attributes but **nothing consumes them** (`processDerives` walks only structs), so `@derive(Eq) enum` was a silent no-op too. Both now error against the closed set (`@derive/@send/@sync/@cLayout`), +2 error fixtures. Nothing in the repo used enum attributes, so rejecting was safe. `@derive(BadTrait)` already errored. | `checker.ts validateAttributes` |
| F7 | **Tests collide on fixed temp paths under concurrent runs** | L | L | Two overlapping `bun test` runs fail ~300 tests against each other — the embedded lane writes a fixed `/tmp/milo_embed_ret.milo`, and the selfhost lane shares `.selfhost/`. Harmless solo (and CI runs one), but it makes any concurrent verification untrustworthy and cost real debugging time twice this session. Use `mkdtempSync` like `tests/fmtCorpus.test.ts`. | `tests/embedded.test.ts` |
| F6 | ~~**`safety.ts` walker never descends into `is` / if- / match-expressions**~~ ✅ fixed 2026-07-16. | H | L | Found by the tsc gate. Arms named fields that don't exist (`IsExpr.expr`→`operand`, `IfExpr.thenBranch/elseBranch`→`thenBody/elseBody`, which are `Stmt[]` needing the stmt visitor) and **`MatchExpr` had no arm at all** — all silently passed `undefined` into a walker that no-ops on falsy input. So at `iec61508-4`, a float in an if-expression branch or match-expression arm was **not flagged** — a false "passed" on the check certification depends on. Proved before/after, fixed, +5 tests (verified all 5 fail without the fix). The comment above the check had claimed full coverage the whole time. | `src/safety.ts` |
| F4 | **Re-enable `noUncheckedIndexedAccess` incrementally** | L | H | Turned **off** 2026-07-16 to make the tsc gate (`tests/typecheck.test.ts`) enforceable at zero. It arrived as a bun-init default and had accumulated ~700 violations, so it enforced nothing; retrofitting them means ~700 `arr[i]!` assertions, which silence the check rather than prove anything (a real fix restructures the loops). Revisit per-file if a class of index bugs ever shows up. | `tsconfig.json` |
| F3 | ~~**Formatter splits `extern struct`**~~ ✅ fixed 2026-07-16. | M | L | `fmt.milo` glued `fn` to a preceding `extern` but not `struct`/`type`, so `struct` read as a top-level item and got a blank line pushed in front — `extern\n\nstruct Foo {`. Had baked itself into **16 committed files** (formatting is applied on the way in). Root cause: `fmt.milo` had **zero test coverage**. Fixed + healed all 16 + added `tests/fmt.test.ts` (targeted, verified to fail without the fix) and `tests/fmtCorpus.test.ts` (corpus-wide idempotence + formatted-check). | `examples/cli-tools/fmt.milo:948` |
| F5 | **`src-milo/` is unformatted** (24 of 25 files) | L | M | The formatted-source gate covers `std/` + `tests/fixtures/` (0 dirty). The self-hosted compiler was never run through `fmt`, plus 3 under `examples/` and 5 under `tests/errors/`. Reformatting `src-milo/` is a big diff that wants the selfhost byte-identity tests (`scripts/selfhost.sh`) watched, so it's its own change. Then widen the gate's `corpus()`. | `tests/fmtCorpus.test.ts` |
