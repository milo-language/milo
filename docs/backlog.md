...
| 10 | **Option `map`** | M | M | `unwrapOrElse` shipped 2026-07-16 (`OptionOp` op `unwrapOrElse`, branch-lowered in `genOptionOp` so the closure runs only on None — `select` would have called it eagerly and defeated the point; fixture `optionUnwrapOrElse.milo` pins the laziness via output ordering). Remaining: `map`, which is the harder half — it must construct `Option<U>`, so it needs `monomorphizeEnum("Option", [U])` plus building the enum value in codegen, not just a phi over an existing payload. Closure call convention: `call <ret> <fnPtr>(ptr <env>, <arg>)` (see `genVecMap`). Copy-inner gate applies as it does to `unwrapOr`. | `checker.ts` Option combinators, `codegen.ts genOptionOp` |
