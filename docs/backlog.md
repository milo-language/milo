...
|---|------|-----|--------|----------------|-----|
| 1 | **`std/smt` is unsound on 64-bit literals** | H | M | The native solver returns **unsat for a satisfiable formula** — a FALSE PROOF, the worst bug a prover can have. Verified against z3, which says sat:<br>`(declare-const x Int)` `(assert (and (>= x (- 9223372036854775808)) (<= x 9223372036854775807)))` `(assert (not (>= x 0)))` — `x = -1` satisfies it. Almost certainly its own i64 arithmetic overflowing on `-2^63`. Any contract with a literal near the i64 bounds can be falsely proven today. Found because `verify.ts` asserts param ranges; i64/u64 are excluded there **only** to dodge this. | `std/smt.milo`, `src/prove-milo.ts` |
| 2 | **Prover models ints as unbounded, params only** | M | H | `verify.ts` now asserts each int param's real range, which killed the `fpMul` false alarms — but **intermediate arithmetic carries no range**, so `error = setpoint - measured` (two i32s) can reach -2^32 in the model and refute a call that no real i32 could. That is the whole `pidStep` baseline entry. Needs range-carrying arithmetic or a bitvector model. | `verify.ts intRangeAssumption` |
