...
|---|------|-----|--------|--------|-----|
| C2 | **No child-exit `Select` arm** | M | M | **The compose half shipped** — `Promise.channel()` exposes the `Channel<T>` a Promise already held, so a `Promise.blocking` OS-thread result now bridges into a Select (`selectPromise.milo` covers both flavors). That needed a third wait tier: with no scheduler there is nothing to park on or poll, so `SelectState` gained a condvar that a foreign pthread's claim signals; `Select.wait` refuses a no-scheduler select with no channel arm rather than blocking on a wake that can never come. **The old blocker was wrong:** `Channel<T>` needs no `clone()` — it is one `*u8` field and therefore implicitly Copy, so `let a = ch; let b = ch` already alias the same channel (checking for a `@copy` attribute or a `clone` method tested the mechanism, not the property). Leftover: a child-exit arm. `SIGCHLD` and `installSignalPipe` both exist now, so what remains is the reap-on-wake pattern — arm the signal fd, and on wake `waitpid(WNOHANG)` the exited child. Unblocks C3. | `std/signal.milo installSignalPipe`, `std/process.milo` |
| C3 | **`timeout` is a `waitpid(WNOHANG)`+sleep poll** | M | M | The one genuine I/O-poll left after the sweep (animation demos are frame timers, correctly untouched): `timeout`'s main loop is `waitpid(pid, status, WNOHANG)` + `sleepMs(50)`. Clean conversion is blocked on the child-exit `Select` arm (C2) — `SIGCHLD` and the Promise-select bridge both shipped, so only reap-on-wake is left. | `examples/cli-tools/timeout.milo` waitpid loop |
...
- **C2's remaining child-exit arm gates C3** (`timeout` conversion); `SIGCHLD` and the Promise-select bridge both shipped, only the reap-on-wake pattern is left.
