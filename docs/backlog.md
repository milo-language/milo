...
|---|------|-----|--------|----------------|-----|
| 1 | **`verify.ts` misattributes unsupported constructs** | L | L | A contract the SMT translator has no arm for (`IfExpr`, `MatchExpr`, `IsExpr`, and 13 more) becomes `(UNSUPPORTED IfExpr)` and reports **`unknown — outside linear fragment`**. Sound (never a false `proven` — checked), but the message blames nonlinearity for a perfectly linear contract, sending you to optimize the wrong thing. Say "translator has no rule for IfExpr" instead. | `src/verify.ts exprToSmt` |
| 2 | **CI tests macOS only** | M | M | `ci.yml` runs `bun test` on `macos-latest` alone; the new `audit-linux` job is the only thing that looks at Linux at all. So `std/*.linux.milo` — epoll, eventfd, the glibc paths — is otherwise **never executed by CI**. `release.yml` builds on ubuntu but runs no tests. Adding `bun test` on ubuntu is the real fix; unknown how much is red today. | `.github/workflows/ci.yml` |
