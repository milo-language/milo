...
| 10 | **Option `map`/`unwrapOrElse`** | M | M | Headline is real — neither exists (`grep unwrapOrElse\|andThen\|orElse\|okOr src std` matches only this entry). **Both stated blockers were stale and are removed:** closures shipped long ago (16 fixtures; `Vec.map` already takes one), and `?`-on-Option already works (checker's `Propagate` case, `isOptionLike` path; fixtures `propagateOption`/`propagateSome`/`optionPropagateDiffT`) — as does `??` (`DefaultValue` case). So it is *unstarted*, not blocked. Shipped: `isSome`/`isNone`/`unwrapOr` (checker's Option-combinator arm), lowered via `OptionOp`. Actual cost, measured: `map` must build `Option<U>` — `monomorphizeEnum("Option", [U])` exists and is already used for enum literals — and `unwrapOrElse` needs **branch-based lowering**, because `genOptionOp` is `select`-based and would call the closure eagerly, defeating the point. Closure call convention: `call <ret> <fnPtr>(ptr <env>, <arg>)` (see `genVecMap`). | `checker.ts` Option combinators, `codegen.ts genOptionOp` |
| 11 | **Flow-sensitive invalidation tracking** | H | H | Compile-time catch of aliased mutation / use-after-invalidate — the aircraft-grade safety tier. | roadmap.md, safety-roadmap.md |
| 12 | **Compile-time reduction** | H | H | Broad DX win; diffuse, needs profiling first (candidate for MIR). | memory: papercuts-from-hades |
