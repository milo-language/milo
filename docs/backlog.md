...
| F3 | **Formatter splits `extern struct`** | M | L | `fmt.milo` doesn't know `extern struct` and emits `extern\n\nstruct Foo {`. Pre-existing and already baked into committed fixtures (`tests/fixtures/externStructNested.milo` is checked in with the mangled form). | `examples/cli-tools/fmt.milo` |
