...
|---|------|-----|--------|----------------|-----|
| 1 | **Annotate std's extern decls with `@cSig`** | M | L | The mechanism shipped; 11 of 215 std extern fns carry `@cSig` (all of `std/sysinfo.darwin`). The rest are still unverified claims. Mechanical, and each one is a real check — the payoff lands when a platform's data model differs (`long` is 4 on LLP64, making a correct-today `i64` decl wrong). Do it opportunistically per module rather than in one sweep. | `std/*.milo` |
