...
|---|------|-----|--------|----------------|-----|
| 1 | **Linux CI is unproven** | M | M | `ci.yml` now has a `test-linux` job running `bun test` + the audit on ubuntu — the first thing ever to execute `std/*.linux.milo`. **It has never run.** It couldn't be verified locally: a container is ~5x slower than the host, so the fixture-compile pool blows its 300s `beforeAll` budget and every test cascades to failure for reasons that say nothing about Linux. Watch the first run; read failures before blaming the job. | `.github/workflows/ci.yml` |
