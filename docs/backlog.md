...
| C6 | **No `AF_UNIX` in `std/net`** | M | M | `std/net` is TCP-only (zero hits for `AF_UNIX`/`UnixListener`/`UnixStream`), so the tmux-style detach/attach daemon uses a localhost TCP port — fine on one box, but a unix socket (filesystem-scoped, no port allocation, peer-cred auth) is the right local transport. **Layout verified by COMPILING on both platforms, not from headers by eye:** darwin `sizeof=106`, `sun_len` u8@0, `sun_family` u8@1, `sun_path[104]`@2; linux `sizeof=110`, `sun_family` u16@0 (no sun_len), `sun_path[108]`@2. `sun_path` is @2 on both; `AF_UNIX`=1 on both. So the struct needs the `std/platform.{darwin,linux}` split but the path offset does not. **The expected blocker is not one:** `std/os` types `bind`/`connect` to `&SockAddrIn`, but a SECOND `extern fn bind(fd, addr: &SockAddrUn, len)` compiles and links fine — `&T` lowers to `ptr`, so the IR declaration is identical. Put the Un externs beside the Un struct rather than refactoring `std/os` to raw pointers. Remaining: `acceptFd(fd)` is already family-agnostic and reusable; `connectFd` is typed to `&SockAddrIn` and needs an Un twin (or a raw-ptr variant) for the green-aware nonblocking connect. | `std/net.milo`, `std/platform.*.milo`, `examples/apps/tmuxDaemon.milo` |
