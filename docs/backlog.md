...
|---|------|-----|--------|----------------|-----|
| 1 | **Extern decls: params + non-libc returns unverified** | M | M | `scripts/audit-extern-returns.ts` checks every scalar-returning `extern fn` against the host's headers with no annotation needed, and **std is clean** (113 checked, 0 findings — so mass-annotating with `@cSig` would find nothing; don't grind it). What's still unchecked: (a) **parameters** — needs a C parser, `@cSig` doesn't cover them either; (b) the 102 skipped — pointer/void returns, and openssl/sqlite3/mach/CoreCrypto which need their own headers; (c) the audit is **host-only**, so `std/*.linux.milo` is unverified on a mac. Run it on Linux too. | `scripts/audit-extern-returns.ts` |
