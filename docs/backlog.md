...
|---|------|-----|--------|----------------|-----|
| 1 | **`extern fn` decls are unverified** | H | M | `@cLayout` closed the `extern struct` hole, but ~341 hand-written `extern fn` signatures are still claims nothing checks — a wrong param type or arity is silently wrong at the ABI seam. Same class, bigger surface. Plausible shape: generate a C TU that takes `&realFn` at the declared signature and let the compiler's own type check reject a mismatch. | `src/checker.ts`, `MILO_PAINPOINTS.md` #8 |
...
| 2b | **`--target` is silently ignored for hosted targets** | H | M | `milo build --target=linux-x64` on macOS reports `compiled ... in 73ms` and emits a **Mach-O arm64 binary** — the host's. `clangTargetFlags` returns `""` for anything not bare-metal, and the link passes `-Wno-override-module`, so clang ignores the IR's triple and builds for the host. Silent wrong artifact, not a failed build. Fixing needs target flags + a sysroot notion (there's no `-I`/`-isysroot` anywhere); failing loudly would already beat this. Blocks meaningful cross-target `@cLayout`/`@cSig` verification, which currently warns and skips. | `src/main.ts clangTargetFlags` |
