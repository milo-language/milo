...
|---|------|-----|--------|----------------|-----|
| 1 | **`std/sysinfo` is darwin-only** | H | M | Every other platform-split std module has both halves; `sysinfo` has only `.darwin.milo`, so **any program importing it cannot compile on Linux** (`examples/apps/sysmon.milo` is the live case). Needs a procfs-backed `sysinfo.linux.milo` — `/proc/meminfo`, `/proc/loadavg`, `/proc/uptime`, `sysconf` for cpu count. Found by the first `test-linux` run. | `std/sysinfo.darwin.milo` |
| 2 | **Linux CI: 3 failures left** | M | M | First `test-linux` run: **965 pass, 7 fail**. Fixed: std/uuid portability, rg's hardcoded regex_t size, the tsc gate's missing `bun install`. Remaining: (a) `std/sysinfo` — item 1; (b) guard test *kills a process tree that exceeds the memory cap* fails on Linux (cgroup/ps accounting differs from macOS — check before weakening, see [[project_selfhost_guard]]); (c) `emit-obj --target=cortex-m3` times out at 5000ms (cold first compile on a slower runner, likely just a budget). | `.github/workflows/ci.yml` |
| 2d | **A caller's `requires` obligation is unverified** | H | H | `requires lo <= hi` is enforced **only when the compiler can const-fold the args** — `clamp(5, 10, 0)` is a compile error, but the same violation via runtime values compiles, runs, and prints a wrong answer silently. `milo prove` emits no VC for the call site either: it proves the callee's `ensures` *given* `requires`, and nobody checks the *given*. So a precondition is an assumption, not a guarantee — the weakest link in the contract story. Options: emit caller-side VCs (real verification), or a debug-mode runtime trap (cheap, catches it at the call). Tested: see the homepage carousel's caption, which is worded around this. | `checker.ts checkCallSiteContracts`, `verify.ts` |
| 3 | **`@cLayout` can't describe synthetic padding fields** | M | M | node-milo's `Rusage` pads with `_p0.._pD` so the struct is big enough for `getrusage` to write 144 bytes — fields that don't exist in C. `@cLayout` emits `offsetof(struct rusage, _p0)` and fails, so that struct can't be verified at all. Wants a way to say "check the first N fields, then trust the size" — the prefix rule already exists for the tail, just not for opaque padding. | `~/git/node/src/milo/bindings/process.milo` |
| 4 | **Struct-field + fn docstring pass** on hot modules | M | L | Bare 6502 register fields (`a/x/y/p`) and un-doc'd fns (`busRead`) read as opaque to newcomers. One-line labels close it. Also seeds `milo doc` (Tier-2). | — |
| 5 | **`std`-shadows-local fix** | M | L→M | A local can be silently shadowed by a std symbol. Correctness + clarity. | memory: papercuts-from-hades |
| 6 | **`src-milo/` is unformatted** (24 of 25 files) | L | M | The formatted-source gate covers `std/` + `tests/fixtures/` (0 dirty); the self-hosted compiler was never run through `fmt`, plus 3 under `examples/` and 5 under `tests/errors/`. Reformatting `src-milo/` wants the selfhost byte-identity tests (`scripts/selfhost.sh`) watched, so it's its own change. Then widen `corpus()`. | `tests/fmtCorpus.test.ts` |
| 7 | **Unused import warnings** — *deferred, not loop-sized* | M | M | Resolver strips imports (`resolveImports` returns `imports: []`), so the checker never sees them — needs threading entry-file imports + used-name collection across resolver→checker, **and** still false-positives on node-milo's link-only imports. Wants a real design pass. | roadmap.md:108 |
...
| 11 | **Iterators** — `.map().filter().collect()` | H | H | Ergonomics everywhere; kills manual index loops. Needs associated types. | roadmap.md:115 |
