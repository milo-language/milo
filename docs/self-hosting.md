...
| M4 self-compile | in progress — **the runaway era is over**: `milo-self check src-milo/main.milo` processes the whole graph (18 files + stdlib) deterministically. Root cause of every "checker runaway" was ONE parser bug: `name<T>(...)` call-site type args weren't a parse form; the misparse cascaded to EOF where the fallback advance no-ops and the call-arg loop spun allocating forever (nondeterminism = getentropy hashmap seeds shifting what got misparsed). Fixed with a type-arg-call lookahead + EOF bail-outs. Whole-graph check now reports 210 ordinary errors (deref-target mutability fixed): top remaining rock is GENERIC STRUCT IMPLS — `impl Channel<T> { fn new/cap/... }` in std/sync isn't instantiated (20 unknown-struct + cascade of undefined-method errors through the runtime graph). (A phantom-imports theory was investigated and RETRACTED — the resolver is correct; a census grep had hit the nonexistent std/event.milo instead of event.darwin.milo.) Then: 20 generic-struct instantiation (Promise/Channel), 10 remaining parse desyncs (`error_unexpected` — likely closures in std), moved-var flow gaps (buf/tn/hintTy), `[u8; N]` array-repeat coercion, Vec.reverse, `_cstrToString` visibility. tokens/ast/hir/lexer/parser self-check clean. | 2026-07-10 |
