...
| M5 convergence | **IN PROGRESS — stage2 grind via ASan.** milo-self2 links + type-checks but its own codegen output is corrupted (loses `cg.body`, drops leading chars: `eclare`←`declare`). Root: opt-level-sensitive **UB** — proven by -O0 (empty) vs -O2 (partial) divergence. **Methodology that works: compile milo-self's self-IR with `clang -O0 -fsanitize=address` and run it; ASan names the exact overflow.** First hit + fixed (`bf3f292`): `genStringEq` ran `memcmp(a,b,aLen)` unconditionally (result AND-ed with lenEq), overflowing the shorter buffer when lengths differ → fixed to `min(aLen,bLen)`. Residual UB remains (more string/buffer bugs at both opt levels; -O0 now trips a lexer error, -O2 loses body via a print/concat corruption). **Next: keep ASan-bisecting stage2** — compile self-IR at -O0 AND -O2 with ASan, fix each overflow, until `milo-self2 emit-ir min.milo` byte-matches `milo-self.bin emit-ir min.milo` (the convergence test). Also backfill manifest fixtures for the behavioral fixes (Vec.pop panic, Heap.clone, ref-mut-through-&mut, short-circuit, string-eq-unequal-len). | 2026-07-10 |
