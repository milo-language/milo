...
| M4 self-compile | in progress — whole-graph `check src-milo/main.milo` down from 218 errors to ~33, all deterministic. Landed: generic struct impls end-to-end (park + instantiate at mono, replay for phase ordering via structInsts), Type<Args>.method and struct-static dispatch, match-arm move isolation, pointer-decay borrows, array-int coercion, runtime-builtin acceptance. Remaining: a residual parse desync inside big impl bodies (repro: de-genericized Channel extract — scratchpad chanx.milo technique: `sed -n '190,330p' std/sync.milo | sed 's/Channel<T>/ChannelX/g; s/<T>/<i64>/g; s/\\bT\\b/i64/g'`, artifact-per-line near the `(*inner).field =` block at cut 64-70, minimal single-assign case passes — context-dependent); 3 immutable-assigns; `?`/`!` on the desync-tainted types. tokens/ast/hir/lexer/parser self-check clean. | 2026-07-10 |
