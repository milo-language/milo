...
| `@mustUse` | `fn`, method, `extern fn` | Discarding the call's result is the `unused-result` warning, as it already is for an `Option` or `Result`; for a `bool` or error code that encodes failure. `let _ = f()` discards on purpose. An error under the DO-178C and NASA safety profiles |
