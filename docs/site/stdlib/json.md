...
Strict RFC 8259 JSON: a parser with typed accessors, a cursor API that navigates without
allocating, and builders (`Json.obj()`, `Json.arr()`) for writing. `Json.parseJsonc`
...
The owned accessors (`get`, `at`) return a subtree as its own `Json`, a deep clone per
call. That reads well and is fine for configuration-sized input; on a large document
navigated in a loop, use the cursor methods (`curRoot`, `curField`, `curChild`,
`curStr`, ...) instead.

<!-- generated:api -->
<!-- Do not edit between these markers: generated by scripts/gen-std-docs.ts from 'milo api --json'. Edit the doc comments in std/json*.milo. -->
...
side-tables the accessors walk. Treat the fields as internal and use the accessors.

Fields: `source: string`, `nodes: Vec<JsonNode>`, `nums: Vec<f64>`, `childIdx: Vec<i64>`, `keyOffsets: Vec<i64>`, `keyLens: Vec<i64>`, `root: i64`.
...
- `Syntax(string)`: The text was not JSON at all — the parser's own message. Path is empty.
- `Missing(string)`: A required field was absent. Payload is the dotted path.
- `Mismatch(JsonMismatch)`: A value was present at the path but of the wrong JSON kind.
...
Fields: `path: string`, `expected: string`, `actual: string`.
...
accessors and jsonNew* constructors below know the layout.
  ...
  number:        a = index into the document's nums table, b = (strStart &lt;&lt; 32) | strLen
  array, object: a = childStart, b = (keyStart &lt;&lt; 32) | childCount (keyStart 0 for arrays)
The packed halves fit because jsonParse caps the input at 2^31-1 bytes and
the pool at 2^32-1 nodes: every count fits the low 32 bits, and every source
offset and key index (at most one per input byte) stays below 2^31, so the
high half never reaches the sign bit. That matters because `>>` on i64 is an
arithmetic shift: a high half of 2^31 or more would unpack negative.

Fields: `kind: i32`, `a: i64`, `b: i64`.
...
Fields: `src: string`, `pos: i64`, `stack: Vec<i64>`.
...
- `JArr(Vec<JsonVal>)`
- `JObj(Vec<JsonMember>)`
- `JRaw(string)`: Pre-serialized JSON spliced verbatim — the caller guarantees validity. Lets builders compose with fragments produced elsewhere (other builders, pass-through payloads) without a parse/rebuild round trip.
