...
| Moving a non-Copy field out of a container element (`let s = v[i].text`) | compile (`cannot move out of index`) | **compile** | `cannot move 'v[...].text' out of 'v': the element stays in the container, so the move would leave a zeroed 'text' behind`; the field was silently zeroed before 2026-09-21 |
