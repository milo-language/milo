...
Generated from `tests/errors/` by `scripts/gen-error-catalog.ts` — 297 distinct messages across 353 pinned programs.
...
- [`'Shards<string>' is not allowed: 'Shards' is @copyOnly and 'string' is not a Copy type (it owns heap memory)`](#shards-string-is-not-allowed-shards-is-copyonly-and-string-is-not-a-copy-type-it-owns-heap-memory)
...
## 'Shards<string>' is not allowed: 'Shards' is @copyOnly and 'string' is not a Copy type (it owns heap memory)

`Shard.get` reads `self.base[i]` through a raw pointer: a bitwise copy of the element. For a `string` that is a second owner of one heap block, and this program (the 2026-09 soundness sweep's hole 1) freed it twice: once when `a` went out of scope and again when the welded Vec was dropped, a malloc abort at HEAD. The element type is decided where `shatter` is called, so that is where it is refused.
...
shatter CONSUMES the Vec. That is the whole aliasing argument: after it there is no binding through which the buffer can be reached except the disjoint windows, so nothing the workers touch can alias anything the caller still holds.
...
<sub>[`tests/errors/shardUseAfterShatter.milo`](../tests/errors/shardUseAfterShatter.milo)</sub>
