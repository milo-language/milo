...
| # | Item | Status / notes |
|---|---|---|
| L1 | In-place arena mutation: `arena.modify(h, (n: &mut T) => ...)` | **exists**: `arenaModifyMut` and `Arena.modifyMut` in `std/arena`. `linkedList.milo` still uses copy-out `arenaModify`. Rewrite the example; consider deprecating the by-value form. |
| L2 | Flatten handle walks: `while let`, `for i, x in v` | **exists** (both). Examples do not use them; see E1. |
...
| T2 | One string story: `String.builder()` or a plus-in-loop lint; byte vs text split in names; `codePoints()` in examples | **partial**: `pushStr`, `codePoints()`, `std/unicode` exist. Lint and example adoption open. |
...
| T4 | JSON: byte-feed/incremental parser; `jq.milo` on `std/json` (`strPath`, cursor API) | **partial**: `strPath` and `curRoot` family exist; `jq.milo` hand-rolls a scanner. |
...
| T6 | Contracts in something people run: `requires`/`ensures` in `std/path`, `kvstore` page math; `milo prove` in CI for them | **partial**: prover and CI gate exist; examples rarely carry contracts. |
...
| # | Item | Status / notes |
|---|---|---|
| E1 | Rewrite the example corpus to CONVENTIONS: `pushStr`, `for x in xs`, `while let`, `arenaModifyMut`, `jq` on `std/json`, no `entries[i].name.clone()` once T1 lands | **open**. Highest-ROI non-compiler item: the corpus is what TS/Go readers actually read. |
| E2 | One short "patterns without lifetimes" example: kv scan, AST handles, sealed spans | **open**. |
| E3 | Fix `return 0 - 1 as i64` and similar in showcase files | **open** (`kvstore.milo:62`). |
...
1. S1-S5 (this session), then E3.
2. L1 adoption + E1 for `linkedList`, `depgraph`, `tree`, `jq`, `kvstore`: proof the model can look good with no compiler work.
3. T1 (`&string` boundaries) + T2 lint: clone tax drops with no new concepts.
