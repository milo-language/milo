...
| F5 | split | `checker.ts` | cheapest seam: thread/global passes, 5 methods, 793 lines, 12 fields + 7 methods, 4 inbound | `checker-program-passes.ts` behind a narrow interface | in progress (WP12 merged 2026-09-20; the split is on its own branch, not yet on main) |
