...
lane C  attributes WP2 → WP3 → WP4                    (checker: attr + move-tracking region)
...
### WP3: `@linear` (closes H2)

Must-consume types. A binding of a `@linear` type that reaches scope end, an early
`return`, or a `?`/`!` unwind without having been moved is a compile error:
`'owner' must be consumed by 'weld' or 'reclaim' before this scope ends`. `forget(x)`
(shipped 2026-08-15) is the explicit escape.

- Checker: extend the drop-glue point that already decides implicit drops (near
  `resourceKind`, line 912, and wherever scope-end drops are scheduled in `lower.ts`)
  to refuse instead of drop. Every exit path counts, including the implicit drop of
  a struct field holding a linear value: a struct with a linear field is itself
  linear (structural, same as Copy).
- Apply to `Shards<T>`. Do not apply to `Sealed` or `Shared` (dropping them is
  sound). Consider `Texture2D` and the GL handles in `examples/games/flight` as the
  second user; if it reads well there, keep it, and delete the "freeing first is not
  optional" comments the attribute now enforces.
- `WeldRejected` and friends already hand the owner back, so a refusal path never
  strands a linear value. Verify with a fixture that welds, is refused, and welds
  again.
- Docs: `residue-vs-rust.md` §2 loses its "keep the owner alive" obligation;
  `ownership-patterns.md` third row of the test shrinks; `std/shard.milo` header.

Done when: `hole2` is an error test and `parallelMap` is documented as convenient,
not as the only safe form.
