...
- **A `&mut T` argument is spelled `foo(&mut x)`** (non-receiver arguments only; `v.push(1)` stays implicit). The marker is stripped by the checker before the argument is checked (`takeExplicitMutArgs`), so borrow rules and codegen never see it. The bare form is the error `implicit-mut-borrow` (`--allow=implicit-mut-borrow` silences it); `bun scripts/explicit-mut.ts [--closure] <file>` rewrites a file from the checker's resolved signatures, `bun scripts/count-implicit-mut.ts` counts what is left across every root and sibling (must stay 0). See `docs/plans/local-reasoning-2026-09.md`.
